...
//  KarmaSide
//
//  Created by Shaan Appel on 4/24/16.
...
class RepliesViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    @IBAction func approveMessagePressed(sender: AnyObject) {
        if let button = sender as? UIButton {
            if let superview = button.superview {
                if let cell = superview.superview as? replyCollectionViewCell {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let cell = receivedMessagesCollectionView.dequeueReusableCellWithReuseIdentifier("repCell", forIndexPath: indexPath) as! replyCollectionViewCell
